CREATE OR ALTER PROCEDURE dbo.EnableNCIndexes
	@DatabaseName SYSNAME,
	@TableSchema SYSNAME,
	@TableName SYSNAME
/******************************************************************************
* Name     : dbo.EnableNCIndexes
* Purpose  : Enables any disabled non-clustered indexes on a specified table
*				by rebuilding them.
...
BEGIN
	...
	SET @TSql = '';
	SELECT @TSql = @TSql + 
		'ALTER INDEX ' + QUOTENAME(JSON_VALUE(j.value, '$.name')) + ' ON ' + QUOTENAME(@DatabaseName) + '.' + QUOTENAME(@TableSchema) + '.' + QUOTENAME(@TableName) + ' REBUILD; ' +
		CHAR(13) + CHAR(10)
